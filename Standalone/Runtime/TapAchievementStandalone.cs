...
namespace TapSDK.Achievement.Standalone
{
    public class TapAchievementStandalone : ITapTapAchievement
    {
        ...
        private async Task PublicAchievement()
        {
            ...
            if (!UrlExistsUsingSockets(TapAchievementAPi.GetHost()))
            {
                NotifyCallbackFailure(
                            achievementId: "",
            ...
            all.ForEach(async (x) =>
            {
                ...
                {
                    if (e is TapAchievementException exception)
                    {
                        switch (x.Type)
                        {
                            case 0:
                                TapAchievementTracker.Instance.TrackFailure("increment", seesionId, x.AchievementId, errorCode: (int)exception.ErrorCode, errorMessage: exception.Message);
                                break;
                            case 1:
                                TapAchievementTracker.Instance.TrackFailure("unlock", seesionId, x.AchievementId, errorCode: (int)exception.ErrorCode, errorMessage: exception.Message);
                                break;
                        }
                        switch (exception.Error)
                        {
                            ...
                            case "access_denied":
                                ...
                                break;
                            default:
                                TapAchievementLog.Log("TapAchievementStandalone -- PublicAchievement Exception1 : " + exception.message);
                                NotifyCallbackFailure(
                                    achievementId: x.AchievementId,
                                    errorCode: TapTapAchievementConstants.UNKNOWN_ERROR,
                                    exception.Message
